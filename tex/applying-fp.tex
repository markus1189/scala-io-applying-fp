...
\begin{frame}[fragile,fragile]
  ...
  \begin{tabular}{p{5cm} l}
    \only<3->{\texttt{List[A]} & is a \texttt{Monoid} \\}
...
def convert[E](o: Option[E]): Xor[E, Unit]
def login(s: String): Xor[LoginError,Token] = for {
...
\begin{frame}
  ...
  \item \texttt{Ior} = no fail/succeed $\rightarrow$ fail/succeed/succeed with warnings
  \end{itemize}
\end{frame}
